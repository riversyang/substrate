...
impl<Block: BlockT<Hash=H256>> Backend<Block> {
		...
		-> Result<(), client::error::Error>
	{
		...
		let imported = if let Some(pending_block) = operation.pending_block {
			...
			let ct_cache_ops = self.changes_tries_storage.commit(
				&mut transaction,
			...
			Some((number, hash, enacted, retracted, displaced_leaf, is_best, cache, ct_cache_ops))
		} else {
			None
		};
		...
		if let Some((number, hash, enacted, retracted, displaced_leaf, is_best, mut cache, ct_cache_ops)) = imported {
			...
			if let Some(ct_cache_ops) = ct_cache_ops {
				self.changes_tries_storage.post_commit(ct_cache_ops);
			}
